<?php
    ...
    function sb_input( $inputdata ) {
        ...
        if ( ! empty( $inputdata['databefore'] ) ) {
            $returndata .= '' . $inputdata['databefore'];
        }

        if ( ! empty( $inputdata['style'] ) ) {
            $styletext .= ' style="' . $inputdata['style'] . '"';
        }
        ...
        if ( $inputdata['type'] == 'text' ) {
            ...
            $returndata .= '<input ' . $styletext . 'autocomplete="off" type="text" id="' . $inputdata['name'] . '" name="' . $inputdata['name'] . '" size="' . $inputdata['size'] . '" maxlength="' . $inputdata['maxlength'] . '" value="' . $inputdata['value'] . '" />';

        } else if ( $inputdata['type'] == 'password' ) {

            $returndata .= '<input ' . $styletext . 'autocomplete="off" type="password" id="' . $inputdata['name'] . '" name="' . $inputdata['name'] . '" size="' . $inputdata['size'] . '" maxlength="' . $inputdata['maxlength'] . '" value="' . $inputdata['value'] . '" />';

        } else if ( $inputdata['type'] == 'checkbox' ) {

            $returndata .= '<input ' . $styletext . 'autocomplete="off" type="checkbox" id="' . $inputdata['id'] . '" name="' . $inputdata['name'] . '" value="' . $inputdata['value'] . '" ';
            $returndata .= ( $inputdata['checked'] ) ? ' CHECKED' : '';
            $returndata .= '/>';

        } else if ( $inputdata['type'] == 'hidden' ) {

            $returndata .= '<input ' . $styletext . 'autocomplete="off "type="hidden" id="' . $inputdata['name'] . '" name="' . $inputdata['name'] . '" value="' . $inputdata['value'] . '" />';

        } else if ( $inputdata['type'] == 'submit' ) {

            $returndata .= '<input ' . $styletext . 'type="submit" value="' . $inputdata['value'] . '" />';

        } else if ( $inputdata['type'] == 'select' ) {

            $returndata .= '<select ' . $styletext . 'autocomplete="off" id="' . $inputdata['name'] . '" name="' . $inputdata['name'] . '"';
    ...
    function sb_test_image_file( $imagefile, $showorreturn = 0 ) {

        //test file
        exec( 'file ' . $imagefile, $filetest );
        $filetest = str_replace( $imagefile . ':', '', $filetest[0] );

        if ( $showorreturn == 0 ) {
            if ( trim( $filetest ) == 'data' ) {
                echo '<div class="redtext alertbox">Image ' . $imagefile . ' is broken.</div>';
            }
        } else {
            if ( trim( $filetest ) == 'data' ) {
                return 0;
    ...
    function sb_test_rw_storage($path) {
    ...
    function sb_check_disks() {
        ...
        foreach ( $output as $item ) {
            if ( strpos( $item, 'mapper' ) == false ) {
                ...
                $diskitems = '--[vda][sda]';
                if ( $settings['firstbackupdisk'] == 'c' ) {
                    $diskitems .= '[vdb][sdb]';
                } else if ( $settings['firstbackupdisk'] == 'd' ) {
                    $diskitems .= '[vdb][sdb][vdc][sdc]';
                }
                if ( ! strpos( $diskitems, '[' . $item . ']' ) !== false ) {
                    $disks ++;
    ...
    function sb_log( $logtext ) {
        ...
        if ( ! empty( $settings['backup_log'] ) ) {
            ...
            if ( ! strpos( " {$lastlogitem}", "{$logtext}" ) !== false ) {
                ...
                exec( 'echo -n "." >> ' . $settings['backup_log'] );
            }
            sleep( 0.5 );
...
';
    ...
    function sb_setting_update( $setting, $value ) {
        ...
        fwrite( $configfile, '"firstbackupdisk" => "' . $settings['firstbackupdisk'] . '",' . "\n" );
